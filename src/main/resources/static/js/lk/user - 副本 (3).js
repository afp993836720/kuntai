...
function submit_deposit() {
	...
	$.post(posturl,postdata,function(res){
		...
		if(res.type == -1){
            ...
            //20180125 七分钱 start
            if (pay_type=='qifenqian_h5_ali'||pay_type=='qifenqian_h5_wx') {
                if(res.indexOf('qfq-error')==0){
                    layer.msg(res);
                }else{
                    location.href = res;
                }
            ...
            if(pay_type == 'qd_znypay2') {

                var gourl = '/index/user/zlypay.html?id=' + res.bpid;
                location.href = gourl;
                can_balance(1);
            }else if(pay_type == 'qipay') {

                    // var gourl = '/index/user/fq7pay.html?id=' + res.bpid;
                    location.href = res;
                    can_balance(1);
            }else if(pay_type.indexOf( 'hyjh_pay')>-1) {

                var gourl = '/index/user/hyjpay.html?id=' + res.bpid;
                location.href = gourl;
                can_balance(1);
            }else if(pay_type.indexOf( 'yyp_pay')>-1) {

                var gourl = '/index/user/yyppay.html?id=' + res.bpid;
                location.href = gourl;
                can_balance(1);
            }
		}
